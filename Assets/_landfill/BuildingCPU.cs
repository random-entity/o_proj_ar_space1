...
public class BuildingCPU : MonoBehaviour // !!!ALL ANGLES IN RADIANS!!!
{
    ...
    {
        angWid = Mathf.Clamp(newValue, BuildingSystemCPU.presetAngWids[BuildingSystemCPU.presetAngWids.Length - 1], Mathf.PI * 2f /*BuildingSystem.presetAngWids[0]*/);
    }
    #endregion
    ...
    public void UpdateMesh(float factor, int roleIndex) // 0 <= factor < 1
    {
        ...
        {
            angWid *= Mathf.Pow(BuildingSystemCPU.angWidStepRatio, -(float)roleIndex + factor);
        }

        float radius = EnvSpecs.landRadius * Mathf.Pow(BuildingSystemCPU.raduisStepRatio, 3 - roleIndex + factor);
        ...
        {
            transform.position = Vector3.up * (radius * angWid * BuildingSystemCPU.angWidToHeight * 4f * (factor - 1f));
        }
        else
        {
    ...
    private void UpdateMesh(float angWid, float radius, bool coverTop) // 논리 기반의 베이스
    {
        ...
        for (int i = 0; i <= res; i++)
        {
            float arg = BuildingSystemCPU.centerDir + angWid * (-0.5f + (float)i / res);
            ...
            y = radius * BuildingSystemCPU.angWidToHeight * angWid;
